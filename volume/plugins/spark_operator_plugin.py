...
class SparkSubmitOperator(BashOperator):
   ...
   """
    ...
    template_fields = ('conf', 'other_spark_options', 'application_args', 'env', 'cluster_id', 'emr_yarn_host', 'cluster_files', 'cluster_jars', 'conf_as_json')
    template_ext = []
    ...
    def execute(self, context):
        ...
        if self.is_not_null_and_is_not_empty_str(self.queue):
            self.bash_command += "--queue " + self.queue + " "
        ...
        if self.is_not_null_and_is_not_empty_str(self.files):
            self.bash_command += "--files " + self.files + " "
